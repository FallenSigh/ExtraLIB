...
int main(void) {
    ...
    std::mt19937 rand;
    std::uniform_int_distribution num(0ull, 31ull);
    rand.seed(19519);
    int m = 0;
    constexpr int n = 100000;

    exlib::unints<64, uint8_t, std::allocator<uint8_t>> a = 0;
    exlib::unints<64, uint32_t> b = 0;
    ...
    auto log_and_check = [&](const char* operation, auto& a, auto& b, auto& c, auto& d, auto action) {
        exlib::log_info("testing {} at {}, {}, {}, {}: ", operation, static_cast<std::uint64_t>(a), static_cast<std::uint64_t>(b), c, d);
        
        if (action() == false) {
            exlib::log_fatal("fatal {} at {}, {}, {}, {}", operation, static_cast<std::uint64_t>(a), static_cast<std::uint64_t>(b), c, d);
            return false;
        }
        return true;
    };                
