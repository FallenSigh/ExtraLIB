...
namespace exlib {
    ...
    template<typename T>
    inline constexpr bool is_integer_v = is_integer<std::decay_t<T>>::value;
}
...
namespace exlib {
    ...
    struct integer {
        ...
        reference assign(self_type&& other) noexcept {
            if (&other != this) {
...
requires std::is_integral_v<I> && exlib::is_integer_v<Int>
